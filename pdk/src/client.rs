...
//!
//! A cosmetic mod declares how content *looks*: a [`VisualModel`] per unit and a
//! per-ability feedback visual ([`EffectVisualDescriptor`], keyed by an
//! [`EffectRole`]). Like the gameplay context it names everything with stable
//! strings that intern to dense handles; [`ClientContext::finish`] emits the
//! [`Names`] tables (only `units` + `abilities` are meaningful to a cosmetic
//! bundle) alongside the accumulated visuals, so the host can map each handle to
//! the gameplay mod's global id by **name** at adoption.
//!
